...
const formItemProps = {
  ...
  labelPosition: {
    type: String,
    validate: (val: string) => ['top', 'right', 'left'].includes(val)
  },
...
export default defineComponent({
  ...
  setup (props, context) {
    ...
    const updateModel = (value: IAnyObject) => {
    ...
    const formItemLabel = () => {
      ...
      const result: Array<VNode|string> = [h('span', { class: { 'is-readonly': props.readonly }, id: labelId }, [formItemConfig.value.label])]
      // 存在说明
    ...
    const renderItemInput = () => {
      ...
      const componentProps = {
        ...
        onStatusChange: (status: boolean) => { // 子组件触发事件控制父组件是否显示
      ...
      if (status.value === 'read-write') {
        const inputComponentProps = {
          ...
          'onStreamUpdate:model': (values: Array<unknown>) => {
            streamUpdateModel(values)
          }
        }
    ...
    const cipFormStyle = computed(() => {
      if (props.grid) {
        const span = formItemConfig.value.span! > props.grid ? props.grid : formItemConfig.value.span // Math.max(props.grid, formItemConfig.value.span)
